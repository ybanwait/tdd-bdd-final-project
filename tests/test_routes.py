...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        # UPDATE THE PRODUCT
        new_product = response.get_json()
        new_product["description"] = "testing2"
    ...
    def test_list_by_name(self):
        ...
        resp = self.client.get(f"{BASE_URL}",  query_string=f"name={quote_plus(test_name)}") 
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        data=resp.get_json()
    ...
    def test_list_by_category(self):
        ...
        resp = self.client.get(f"{BASE_URL}",  query_string=f"category={test_category.name}") 
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        data=resp.get_json()
        self.assertEqual(len(data), count)
        for product in data:
    ...
    def test_list_by_availability(self):
        ...
        resp = self.client.get(f"{BASE_URL}",  query_string=f"available=true") 
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        data=resp.get_json()
        self.assertEqual(len(data), count)
        for product in data:
